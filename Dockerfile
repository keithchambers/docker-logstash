...
RUN opkg-install bash curl grep ca-certificates \
    && curl -Lskj -b "oraclelicense=accept-securebackup-cookie" http://download.oracle.com/otn-pub/java/jdk/8u40-b26/jre-8u40-linux-x64.tar.gz | gunzip -c - | tar xf - -C /opt \
    && rm -rf $(find /opt/ | grep -E "/jre1.8.0_40/lib/(desktop|locale|fonts|plugin)/") \
    && ln -s /opt/jre1.8.0_40/bin/java /usr/bin/java \
    && curl -Lskj "http://download.elastic.co/logstash/logstash/logstash-$LOGSTASH_VERSION.tar.gz" | gunzip -c - | tar xf - -C /tmp \
